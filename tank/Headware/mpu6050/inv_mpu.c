...
u8 mympu_get_quat(float *q0,float *q1,float *q2,float *q3)
{
	float Q0=1.0f,Q1=0.0f,Q2=0.0f,Q3=0.0f;
	unsigned long sensor_timestamp;
	short gyro[3], accel[3], sensors;
...
u8 mympu_get_accel(float *AX,float *AY,float *AZ)
{
	float Q0=1.0f,Q1=0.0f,Q2=0.0f,Q3=0.0f;
	unsigned long sensor_timestamp;
	short gyro[3], accel[3], sensors;
	...
	Accel_X_RAW=accel[0]+X_Accel_OFFSET;
	Accel_Y_RAW=accel[1]+Y_Accel_OFFSET;
	Accel_Z_RAW=accel[2]+Z_Accel_OFFSET;
...
void mympu_cal_angle(float *pitch,float *roll)
{
	float AX,AY,AZ;
	mympu_get_accel(&AX,&AY,&AZ);
	*pitch = atan(AY/AZ)*180/3.14;
	*roll = atan(AX/AZ)*180/3.14;
}

void mympu_cal_quat(float *pitch,float *roll,float *yaw)
{
	float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;
	mympu_get_quat(&q0,&q1,&q2,&q3);
	*pitch = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;	// pitch
	*roll  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;	// roll
	*yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
}
